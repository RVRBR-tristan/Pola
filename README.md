...
- **Films** : 600, SX-70, Time Zero, Go, 669, Portra 160, Portra 400, Ektachrome, Kodachrome, N&B 667, Bleu (Reclaimed), Duochromes Bleu/Jaune/Rouge, Expiré — calibrés d'après le catalogue Polaroid / Kodak et les caractéristiques documentées (les Portra approchent les négatifs couleur Kodak, doux et chauds) ; chaque preset combine voile des noirs et plafond des blancs par canal, croisement tonal, saturation, halation, grain et vignettage, appliqués pixel par pixel sur canvas.
...
- **Réglages** : exposition (± 1,2 EV), contraste, saturation, grain, flou gaussien et vignettage — repliés dans le pipeline film, aperçu rapide basse résolution pendant le glissement, rendu plein au relâchement. Saturation, grain et vignettage s'initialisent sur le film choisi. Onglets Réglages / Fond 4:5.
...
- **Double exposition** : dans Réglages, superpose une seconde photo importée sur le tirage, avec une intensité réglable et trois modes de fusion — Éclaircir (screen, le look film classique), Assombrir (multiply) ou Fondu. Le mélange est appliqué avant le rendu film (traitement unifié) et repris à l'export ; conservé avec le tirage dans la galerie.
- **Double exposition à la capture** : une icône dans l'en-tête du viseur active le mode ; on prend une première photo, puis un calque fantôme de celle-ci apparaît dans le viseur (fusion « screen » en direct) pour aligner la seconde. Le déclencheur suivant assemble les deux et ouvre l'éditeur avec le menu Double expo déjà ouvert.
...
- **Couleur du fond 4:5** : blanc, noir, ou une couleur perso prélevée sur la photo — la **pipette** (à gauche des pastilles blanc / noir) s'arme puis un tap sur l'aperçu échantillonne le pixel touché ; la couleur choisie est conservée avec le tirage et reprise à l'export.
